  ...
  try {
    await axios({
      ...
      headers: {
        ...
        'Accept-Language': localization?.getLanguage() || 'en',
...
export const POST_FORM_WITHOUT_DATA = async (
  route,
  body,
  onSuccess = () => { },
  onError = () => { },
...
export const GET = async (
  ...
  try {
    // console.log(localization.getLanguage() ,"localization.getLanguage() ")
    axios({
      method: 'get',
      url: `${API}${route}`,
      headers: {
      ...
      .then(res => {
        if (res?.status == 200 || res?.status == 201) {
          ...
          if (statusMessage[res?.status]) {
            SimpleToast.show(`${statusMessage[res?.status]}`);
          }
          onError(res);
          return res;
        }
